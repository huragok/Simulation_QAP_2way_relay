...
load('Test_201572411838534') % 64QAM
...
for i_sigma2 = 1 : n_sigma2
    ...
    EMI_analytical{i_sigma2} = zeros(M, 1);
    EMI_analytical{i_sigma2}(:, 1) = get_EMI_lower_bound(constellation, map_QAP{i_sigma2}, beta_sr, beta_rd, g(i_sigma2), sigma_sqr_d(i_sigma2), sigma_sqr_r(i_sigma2));
    
    % Compute the ergodic mutual information using Monte-Carlo simulation
    EMI_MC{i_sigma2} = zeros(M, 1);
    
    EMI_MC{i_sigma2}(:, 1) = get_EMI(constellation, map_QAP{i_sigma2}, beta_sr, beta_rd, Pr, Pt, Pt, sigma_sqr_d(i_sigma2), sigma_sqr_r(i_sigma2), N_per_batch, N_batch, seed);
    
    toc;
    disp(['EMI simulation for 1/sigma2 = ', num2str(dB_inv_sigma2(i_sigma2)), 'dB completed.'])
    disp([' - EMI lower bounds, QAP: ', num2str(EMI_analytical{i_sigma2}(:, 1)')])
    disp([' - EMI emperical, QAP: ', num2str(EMI_MC{i_sigma2}(:, 1)')])
end

EMI_analytical = reshape(cell2mat(EMI_analytical), M, 1 * n_sigma2);
EMI_MC = reshape(cell2mat(EMI_MC), M, 1 * n_sigma2);
...
legend_item{1} = 'TR0';
for m = 2 : M
    plot(dB_inv_sigma2, EMI_analytical(m, 1 : n_sigma2), 'o-.', 'Color', cmap(m, :), 'linewidth', 2), hold on;
    legend_item{m} = ['QAP', num2str(m-1)];
end
grid on;
set(gca, 'Fontsize', 18);
xlabel('1/\sigma^2(dB)'), ylabel('EMI');
legend(legend_item, 'Location', 'northeast');
saveas(h, ['EMI_noise_power_upperbound_BER_', num2str(Q), 'QAM.fig']);
...
legend_item{1} = 'TR0';
for m = 2 : M
    plot(dB_inv_sigma2, EMI_MC(m, 1 : n_sigma2), 'o-.', 'Color', cmap(m, :), 'linewidth', 2), hold on;
...
xlabel('1/\sigma^2(dB)'), ylabel('EMI');
legend(legend_item, 'Location', 'northeast');
saveas(h, ['EMI_noise_power_MonteCarlo_BER_', num2str(Q), 'QAM.fig']);