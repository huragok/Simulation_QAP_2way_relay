...
{	int		max_iter, dec_type;
	...
	LLR_output = calloc( max_iter*CodeLength, sizeof( float ) );
	...
	if ( dec_type == 1) {
		...
		SumProduct( BitErrors, DecodedBits, LLR_output, c_nodes, v_nodes, CodeLength, 
		   NumberParityBits, max_iter, r_scale_factor, q_scale_factor, data_int ); 
	}
    ...
    for (i=0;i<max_iter;i++) {
		for (j=0;j<CodeLength;j++) {
			output_p[i + j*max_iter] = LLR_output[i+j*max_iter];
		}
	}
