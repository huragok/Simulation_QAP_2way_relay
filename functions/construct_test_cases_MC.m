function test_cases = construct_test_cases_MC(Nbps, type_mod, dB_inv_sigma2, p_Pr, d, nu, M, verbose, N_batch, N_per_batch, seed)
%   test_cases = construct_test_cases_MC(Nbps, type_mod, dB_inv_sigma2, p_Pr, d, nu, M)
...
for i_d = 1 : n_d
    for i_p_Pr = 1 : n_p_Pr
        ...
        for i_sigma2 = 1 : n_sigma2
            ...
            % Q-by-Q matrix
            test_cases(i_case).param_derived.E = get_factor_PEP_update_MC(dist_sqr, beta_sr(i_d), beta_rd(i_d), Pt(i_p_Pr), Pr(i_p_Pr), sigma_sqr(i_sigma2), sigma_sqr(i_sigma2), N_batch, N_per_batch, seed); % Get E with Monte-Carlo Simulation
